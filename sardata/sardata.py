...
class SARData():
    ...
    def __init__(self,date_str=None,LLX=-19.7, LLY=63.8, LRX=70.0, LRY=63.8, \
                 URX=70.0, URY=82.3, ULX=-19.7, ULY=82.3):
        ...
        self.month = mm
        self.day = dd
        if (isinstance(LLX,(float,int)) & isinstance(LLY,(float,int)) & \
            isinstance(LRX,(float,int)) & isinstance(LRY,(float,int)) & \
            isinstance(ULX,(float,int)) & isinstance(ULY,(float,int)) & \
            isinstance(URX,(float,int)) & isinstance(URY,(float,int)) == False):
    ...
    def get_NBS_ColhubData(self):
        ...
        cmd = 'wget --no-check-certificate --user=%s --password=%s --output-document=%s ' % (USER_NBS, PASSWD_NBS, xmlFile)
        ...
        for node in entrys:
            fname = '%s/%s.zip' % (RAWDIR,node.getElementsByTagName('title')[0].childNodes[0].nodeValue  )
        ...
        for i in range(len(proclist_tmp)):
            ...
            if (processedLog.find(fsub) == -1): #Check if file processed before
                if (str(proclist).find(fsub) == -1): #Check if file already put to proclist ?
                    ...
                    (USER_NBS, PASSWD_NBS, proclist_tmp[i])
                    cmd = cmd + '\"%s\" ' % (val)
                    subprocess.call(cmd, shell=True)
                    print (cmd)
